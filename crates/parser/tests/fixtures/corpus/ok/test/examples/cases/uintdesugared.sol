...
contract Uint {
   ...
   function main() public returns (uint) {
      ...
      mint(uint(1000));
      mint(uint(1000));
      return rval(IndexAccessProxy(LValueMemberAccess.memberAccess(MemberAccessProxy(ContractStorage(UintCxt), balances_sel)), rval(MemberAccessProxy(ContractStorage(UintCxt), owner_sel)))) ;
   }
}
