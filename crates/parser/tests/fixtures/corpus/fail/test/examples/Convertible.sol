...
impl Typedef<uint16, word> {
  function abs(r:word) returns (uint16) { return uint16(r);}
...
impl Typedef<uint8, word> {
  function abs(r:word) returns (uint8) { return uint8(r);}
...
impl Typedef<uint256, word> {
  function abs(r:word) returns (uint256) { return uint256(r);}
...
impl Convertible<Pair<uint8, Proxy<uint16>>, uint16> {
    function convert(p: Pair<uint8, Proxy<uint16>>) returns (uint16) {
       match (p) {
case Pair(x, _) {
return Typedef.abs(Typedef.rep(x));
}
}
    }
}
