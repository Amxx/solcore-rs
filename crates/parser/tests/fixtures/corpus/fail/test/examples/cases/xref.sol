...
trait Ref<r, d> { function load(x: r) returns (d) ; function store(x: r, v: d) ;}
...
impl<a> Typedef<M<a>, word> {
  function rep(m: M<a>) returns (word) { match (m) {
case M(w) {
return w;
}
}}
...
trait MemoryType<Self> {
  ...
  function memoryStep(offset: word, self: Self) returns (word) ;
...
} // match-compiler cannot compile mopre than 1 stmt in a branch :(
  }
}
...
impl<a, b, r> MemoryRef<XRef<r, PairSnd, b>, b> where r: MemoryRef<(a, b)>, a: MemoryType, b: MemoryType {
  function addr(xr: XRef<r, PairSnd, b>) returns (word) {
    match (xr) {
case XRef(r, _) {
return add_(MemoryRef.addr(r), MemoryType.memorySize(@b));
}
}
  }
}
