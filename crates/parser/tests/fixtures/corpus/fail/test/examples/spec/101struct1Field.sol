...
impl MemoryType<uint> {
    function load(ptr: word) returns (uint) {
        return Typedef.abs(mload_(ptr)); // type annotation needed due to a typechecker bug
    }
