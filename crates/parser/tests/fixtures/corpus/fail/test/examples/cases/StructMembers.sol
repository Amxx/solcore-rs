...
impl Ref<MemberAccess<Memory<structType>, fieldType, Memory<ty>>, ty> {
    function load(x: MemberAccess<Memory<structType>, fieldType, Memory<ty>>) returns (ty) {
        let ptr:Word;
        match (x) {
case MemberAccess(Memory(y)) {
ptr = y;
}
}
