...
forall a . instance memory(a):MemoryType {
    function load(ptr:word) -> memory(a) {
        return Typedef.abs(mload_(ptr));
    }
    function store(ptr:word, value:memory(a)) -> () {
        return mstore_(ptr, Typedef.rep(value));
    }
}
...
forall a
. instance memory(a):MemorySize {
    function size(x:Proxy(memory(a))) -> word {
        return 32;
    }
}
...
forall structType fieldSelector fieldType offsetType
  ...
  => instance MemberAccessProxy(memory(structType), fieldSelector, offsetType):RValueMemberAccess(fieldType) {
    function memberAccess(x:MemberAccessProxy(memory(structType), fieldSelector, offsetType)) -> fieldType {
        ...
        return MemoryType.load(ptr):fieldType;
    }
}
...
instance StructField(W, flds_sel):CStructField(memory(S), ()) {}

function makeS() -> memory(S) {
    ...
    let fld3_map = MemberAccessProxy(s, fld3_sel)
		 : MemberAccessProxy(memory(S), fld3_sel, (uint,word));
    // let y:word = 13;
    let z:uint = uint(13);
...
function readS(s:memory(S)) -> word {
    let s:memory(S) = Typedef.abs(0x80);
    let fld1_map : MemberAccessProxy(memory(S), fld1_sel, ()) = MemberAccessProxy(s, fld1_sel);
    let fld2_map : MemberAccessProxy(memory(S), fld2_sel, uint) = MemberAccessProxy(s, fld2_sel);
    let fld3_map = MemberAccessProxy(s, fld3_sel)
...
function makeW(s:memory(S)) -> memory(W) {
...
function readW(w:memory(W)) -> memory(S) {
    let flds_map : MemberAccessProxy(memory(W), flds_sel, ()) = MemberAccessProxy(w, flds_sel);
