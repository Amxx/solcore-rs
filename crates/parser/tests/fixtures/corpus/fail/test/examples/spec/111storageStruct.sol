...
impl StorageType<uint> {
    function sload(ptr: word) returns (uint) {
        return Typedef.abs(sload_(ptr)); // type annotation needed due to a typechecker bug
    }
...
impl<structType, fieldSelector, fieldType, offsetType> RValueMemberAccess<MemberAccessProxy<storage<structType>, fieldSelector, offsetType>, fieldType> where StructField<structType, fieldSelector>: CStructField<fieldType, offsetType>, fieldType: StorageType, offsetType: StorageSize {
    function memberAccess(x: MemberAccessProxy<storage<structType>, fieldSelector, offsetType>) returns (fieldType) {
        let ptr:word = Typedef.rep(memberAccessD1(x));
        let size:word = StorageSize.size(@offsetType);
        assembly {
            ptr := add(ptr, size)
