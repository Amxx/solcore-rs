function add1(x: Word) returns (Word) {
  return addW(x,1);
}
...
function lam1impl<f>(env: f, a: Word) returns (Word) where f: Invokable<Word, Word> {
  let f = env;
...
*/
impl<f> Invokable<Lam1Closure<f>, Word, Word> where f: Invokable<Word, Word> {
  function invoke(clos: Lam1Closure<f>, arg: Word) returns (Word) {
    match (clos) {
case Lam1Closure(env) {
return lam1impl(env, arg);
}
}
  }
}

function add1mod<f>(f: f) returns (Lam1Closure<f>) where f: Invokable<Word, Word> {
