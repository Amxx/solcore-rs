...
// impls for local data types (alongside their Generic impl). It carries
...
// StorageSize / StorageType / CanStore traits how to deal with the
// primitive SOP types that `Generic` maps user data types onto
//   sum(f, g)  with constructors inl / inr   (choice / tagged union)
//   (f, g)     pair                          (product)
//   ()         unit
// and then bridges every type with a `Generic<rep>` impl to those
// layouts. `Generic` impls are auto-derived for local data types, so
...
//
// The structural impls above teach StorageType the fixed-slot encoding of
...
// Solidity-style) and a CanStore impl (`storage<bytes>: CanStore<memory<bytes>>`)
// but NO StorageType impl. Routing an ADT's storage through StorageType
...
// representation and storing each leaf through the leaf's OWN CanStore impl.
...
let base : word = Typedef.rep(r);
            let a_sz : word = StorageSize.size(@a);
            let xSlot : storage<a> = storage(base);
            let ySlot : storage<b> = storage(base + a_sz);
            CanStore.store(xSlot, x);
            CanStore.store(ySlot, y);
}
}
    }
    function load(r: storage<(a, b)>) returns (a, b) {
        ...
        let xSlot : storage<a> = storage(base);
        let ySlot : storage<b> = storage(base + a_sz);
        let x : a = CanStore.load(xSlot);
        let y : b = CanStore.load(ySlot);
        return (x, y);
    }
}
...
case inl(x) {
sstore(base, 0);
            let slot : storage<f> = storage(base + 1);
            CanStore.store(slot, x);
}
case inr(y) {
sstore(base, 1);
            let slot : storage<g> = storage(base + 1);
            CanStore.store(slot, y);
}
}
    }
...
case 0 {
let slot : storage<f> = storage(base + 1);
return inl(CanStore.load(slot));
}
default {
let slot : storage<g> = storage(base + 1);
return inr(CanStore.load(slot));
}
}
    }
}
...
// / storage<string> impls (data lives at keccak(slot)); these mirror them at
...
impl CanStore<storage<memory<bytes>>, memory<bytes>> {
    function store(r: storage<memory<bytes>>, v: memory<bytes>) {
        let slot : storage<bytes> = storage(Typedef.rep(r));
        CanStore.store(slot, v);
    }
...
impl CanStore<storage<memory<string>>, memory<string>> {
    function store(r: storage<memory<string>>, v: memory<string>) {
        let slot : storage<string> = storage(Typedef.rep(r));
        CanStore.store(slot, v);
    }
...
//
// A `default impl<a> StorageType<a>` would have its `load` return the head
// variable `a` via Generic.to — but the specializer cannot monomorphize a
// result-position type variable of a default impl (it is not pinned by the
...
//
// Instead, DeriveGeneric emits a concrete, per-type
// `storage<T>: CanStore<T>` impl (see Solcore.Desugarer.DeriveGeneric) where the
// data type is fixed in the impl head; it delegates to the structural CanStore impls above
...
// read back any `a` that has a `Generic<rep>` impl at a raw storage slot.

function storeGeneric<a, rep>(slot: word, value: a) where a: Generic<rep>, rep: StorageType {
